...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn,
  ...
  *mn = an;
  ...
  for (j = 0; j < an; ++j) {
    // TODO: for every descriptor in a, find best match in b.hh
    int bind = 0;
    m[j].ai = j;
    m[j].p = a[j].p;

    float min_distance =
        l1_distance(a[j].data, b[0].data, a[j].n < b[0].n ? a[j].n : b[0].n);
    for (i = 0; i < bn; ++i) {
      float distance =
          l1_distance(a[j].data, b[i].data, a[j].n < b[i].n ? a[j].n : b[i].n);
...
point project_point(matrix H, point p) {
  ...
  matrix projected_point = matrix_mult_matrix(H, c);
  point q = make_point(projected_point.data[0][0] / projected_point.data[2][0],
                       projected_point.data[1][0] / projected_point.data[2][0]);
  return q;
}
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff) {
  ...
  for (int i = 0; i < k; i++) {
    ...
    matrix Homo = compute_homography(m, 4);
    if (Homo.data) {
      int inliers = model_inliers(Homo, m, n, thresh);
      if (inliers > best) {
        best = inliers;
...
image cylindrical_project(image im, float f) {
  ...
  for (int i = 0; i < im.w; i++) {
    for (int j = 0; j < im.h; j++) {
      float degree = (i - cx) / f;
      float h = (j - cy) / f;
      float x_new = sinf(degree);
      float y_new = h;
      float z_new = cosf(degree);
      float x_updated = f * x_new / z_new + cx;
      float y_updated = f * y_new / z_new + cy;
      for (int k = 0; k < im.c; k++) {
        if (x_updated >= 0 && x_updated < im.w && y_updated >= 0 &&
            y_updated < im.h) {
          float val = get_pixel(im, x_updated, y_updated, k);
